...
1. I was permitted access to the **real BAPE research model and training repository**
2. I rebuilt the naive single-EC2 arch from Phase 2 as a **securely isolated, load-balanced VPC architecture** — deployed by hand via the AWS CLI – deliberately, before adopting Infrastructure-as-Code in later phases.
...
more self-documenting object stating inference metadata, estimated parameters (T60), quantiles and latents (the acoustic fingerprint `z`).
...
**Consequences.** Correct end-to-end inference locally against the FastAPI wrapper, and an API contract that carries meaning rather than raw numbers.
...
**Rationale.** `10.16.0.0/16` deliberately avoids the commonly-used low `10.x` ranges, like `10.1.x`, `10.2.x`., …, to reduce the
...
**Decision.** Comply with the ALB's two-AZ requirement: a fully subnetted two-AZ VPC — **two public subnets** (ALB nodes, NAT Gateway) and **two private subnets** — but run only **one** EC2 instance, leaving the load balancer with a single healthy target.
...
**Rationale.** An open SSH port is an attack surface. SSM gives auditable, key-less access
...
**Consequences.** I developed a deeper familiarity with the inner workings of my infra and felt the pain: to save on costs I tore down cost-generating resources (NAT Gateway, ALB, EC2) **at the end of every session and rebuilt on return**. Keeping momentum when rebuilding was only possible through meticulous documentation of all commands and resulting attributes. That friction made the move to Infrastructure-as-Code in the following phases a no-brainer.
...
- **Validated infrastructure:** 
  ...
  - an ALB spanning both AZs (one node per), a NAT Gateway, and a single `t3.small` (20 GB gp3) EC2 instance
  loading the model from S3 at boot via `user-data` shell script
  - static frontend built on Tailwind CSS
