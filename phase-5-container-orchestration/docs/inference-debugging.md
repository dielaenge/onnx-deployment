...
- In my exporter script this is still the case and as it is till broken for phase 5 because of missing pathlib logic, we need to fix this next (`PROJECT_ROOT` and `SCRIPT_DIR` + `ESTIMATOR_WEIGHTS_PATH`, `ENCODER_WEIGHTS_PATH`, `REF_AUDIO_PATH` and `EXPORTED_ONNX_PATH`)

- rerunning exporter script from phase5 
  - missing dependcies: omegaconf
  - fixing links in vendored bape source
  - failing because the operator `aten::_transformer_encoder_layer_fwd` could not be exported to any opset version with dyanmic batching enabled (`dynamo = False`)
    - this didn't happen before because part of the bug fix was to initialize the `encoder_state` in the SuperParameterEstimator instance used in the onnx export script
    - as the encoder heads were not used, the `aten::_transformer_encoder_layer_fwd`operator was not used, thus not exported and the export didn't crash
      - fix: in phase 5 we don't want to do parallel batch inference processing of dynamic shapes [N, 1, 16, 2000], but continuous sequential stream processing, this means the inference engine only accepts static input shapes [1,1,16,2000]

end of april 13
- reiterated onnx export: no dynamic batch sizing required as model ever only proceeds one spectrogram at a time during streamed processing
- export based on unit test 

- onnx exports opset version 18 

- slicing functionality in the current version / phase can only start when the JS frontend has received the entire (finished) recording --> it is not set up for real-time processing; Lambda Function URL not capable 
  - decision: 
    1. deploy as is to ECS to make shift to new infrastructure
    2. update from HTTP to WebSockets / update application protocol
      > [HTTP vs WebSockets](https://blog.postman.com/websockets-vs-http-key-differences-explained/)
    - read and enrich

- refactored inference_engine.py to loop through static size inputs, `single_input` of shape [1,1,16,2000] stacked in `batch_inference_input` of shape [N,1,16,2000] 

- overall I assume the Python loop is a rather resource-inefficient way to run sequential inference, but would need to do further research; question would be **What are alternative ways of running sequential inference sessions instead of using a Python loop? How do they benchmark? What use cases are limited by this decision?**

- updated Dockerfile and built `bape-ph5-std-fix` image – unsure of my naming convention…