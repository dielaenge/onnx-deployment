...
def step(model, batch, target, device, loss_fn: Callable) -> dict:
    _, spec, _, param = batch
    ...
    return {"loss": loss, "est": est, "gt": gt}


@hydra.main(config_path="../conf", config_name="train_param_nb", version_base=None)
def main(cfg: DictConfig) -> None:
    ...
    subdir = cfg.subdir if cfg.subdir is not None else "nb"
    log_dir, out_dir = create_log_dir(subdir=subdir, debug=cfg.debug)
    ...
    try:
        for epoch in range(cfg.trainer.max_epochs):
            ...
            with tqdm(
                ...
                for batch in pbar:
                    optimizer.zero_grad()
                    step_dict = step(model, batch, target, device, loss_fn)
                    loss = step_dict["loss"]
                    loss.backward()
                    optimizer.step()
